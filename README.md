...
| `verba verbix <cmd>` | Run Verbix package manager via verba |
...
Verba has built-in support for threads, parallel blocks, and worker pools.
...
- `thread <name> = run <func> [with <args>].` — start a background thread
- `join <name>.` — wait for one thread
- `join all t1, t2, t3.` — wait for multiple threads

### Parallel Block

Runs each `run` statement inside the block concurrently, then waits for all to finish.
...
A fixed-size pool of worker threads that processes tasks from a queue.
...
- `worker <name> = <n> workers.` — create a pool with N threads
- `send <pool> do <func> [with <args>].` — enqueue a task
- `wait for <pool>.` — block until all tasks finish
...
```verba
run express.use with "static_dir", "/static".
...
### `verbix` — Package Manager (built-in)

Verbix is also available as a built-in module inside every Verba script.

```verba
/- Install a package at runtime
msg = the result of running verbix.install with "mathkit".
say msg.
...
Verbix is the official package manager for Verba. It manages `.vrb` packages in the local `modules/` folder and supports full semantic versioning.

### Running Verbix

```bash
# Standalone (after pip install -e .)
verbix install mathkit

# Via Python module
python -m verbix install mathkit

# Via verba subcommand
verba verbix install mathkit
```
...
| Command | Description |
|---|---|
| `verbix install <name[@ver]>` | Install a package by name or `name@version` |
| `verbix install <url>` | Install directly from a URL |
| `verbix uninstall <name>` | Remove an installed package |
| `verbix upgrade <name>` | Upgrade a package to its latest version |
| `verbix upgrade all` | Upgrade all installed packages |
| `verbix outdated` | Show packages with newer versions available |
| `verbix packages` | List all installed packages with versions |
| `verbix search <query>` | Search the registry by name or description |
| `verbix info <name>` | Show installed version, latest, and all available versions |
...
verbix install mathkit@1.0.0

# Check what's outdated
verbix outdated

# Upgrade one package
verbix upgrade mathkit
...
Installed packages land in `modules/` and are tracked in `modules/.registry.json`.
...
### Registry format (`verba_packages/index.json`)
...
```
verba/
├── ast.py              — AST node definitions (incl. concurrency nodes)
├── tokenize.py         — Lexer / tokenizer
├── parser.py           — Parser (tokens → AST)
├── runtime.py          — Interpreter / evaluator
├── runtime_types.py    — Runtime types (Environment, Function, VerbaThread, VerbaWorkerPool, …)
├── errors.py           — VerbaParseError, VerbaRuntimeError
├── cli.py              — CLI entry point (run, check, repl, format, verbix)
├── verbix_cli.py       — Verbix package manager logic
├── pkg_registry.py     — Local registry (modules/.registry.json)
└── stdlib/
    ├── strings.py
    ├── math.py
    ├── json.py
    ...
    ├── gui.py
    ├── express.py
    └── pkg.py          — verbix built-in module

verbix/                 — Standalone verbix package (python -m verbix)
├── __init__.py
└── __main__.py

verba_packages/         — Local package registry server files
├── index.json          — Multi-version package index
├── mathkit-1.0.0.vrb
├── mathkit-2.0.0.vrb
└── mathkit.vrb

modules/                — Installed packages (created on first install)
└── .registry.json      — Tracks installed packages and versions

verbix_main.py          — Entry point for verbix CLI
verbix.bat              — Windows batch launcher
verbix.config.json      — Registry URL configuration
verbix.spec             — PyInstaller spec for verbix.exe
```
...
| `examples/concurrency.vrb` | Threads, parallel block, worker pool |
| `examples/use_mathkit.vrb` | Using a Verbix-installed package |
| `examples/use_verbix.vrb` | verbix_utils module demo |
...
python -m verba run examples/concurrency.vrb

# Verbix package manager
verbix install mathkit
python -m verba run examples/use_mathkit.vrb
